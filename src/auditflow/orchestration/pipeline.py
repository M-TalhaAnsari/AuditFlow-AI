...
class Sessions:
    ...
    def reload(self) -> None:
        """Rebuild chunk_lookup and all_identities from the current
        corpus (FAISS/BM25 must already have been reloaded by
        retrieve.reload_index_from_disk() before this runs
        """
        new_chunk_lookup = get_full_chunk_lookup()
        new_identities = get_all_document_identities()

        with self._reload_lock:
            self.chunk_lookup = new_chunk_lookup
            self.all_identities = new_identities

        logger.info(
            "Sessions() singleton reloaded: %d chunks, %d document identities",
            len(new_chunk_lookup), len(new_identities),
        )
