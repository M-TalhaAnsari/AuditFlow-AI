...
class Sessions:
    ...
    passed into ask(), NOT on self. What IS on self (chunk_lookup,
    all_identities) is corpus-wide, read-only, shared-by-design data --
    identical for every user, safe to build once at process startup.
 
    This fixes ARCHITECTURE.md known limitation #1 for real: previously
    self.active_contract/self.pending_question were shared across every
    concurrent user of this singleton. Now every method that reads or
    writes conversational memory takes `state: SessionState` explicitly
    and touches nothing on self -- so two users hitting this same
    Sessions() instance at once can't step on each other's state."""
    ...
    @staticmethod
    def _mentions_different_contract(question: str, identities: dict, active_contract: str | None = None) -> str | None:
        """Checks if the question names a document (by either party's
        name) different from the active one, via real company_name /
        counterparty_name fields from identity extraction. Pure function
        of its arguments -- no Postgres/FAISS needed, so it's testable
        without constructing a real Sessions() instance."""
    ...
    def _handle_low_confidence(self, question: str, consistency, state: SessionState) -> AskResponse:
        ...
        mentioned = self._mentions_different_contract(question, self.all_identities, state.active_contract)
